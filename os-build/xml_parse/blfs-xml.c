...
//对于xmlChar* xch=BAD_CAST("book")，char* ch=(char *)(xch)
		...
		if (!strcmp(name, "cmd")) {
			if (!xmlStrcmp(node->name, BAD_CAST("para")) &&
			    !xmlStrcmp(node->parent->name, BAD_CAST("listitem"))) {
				printf("%s\n", ((char *)
						XML_GET_CONTENT(node->xmlChildrenNode)));
				if (node->children->next && !xmlStrcmp(node->children->next->name, BAD_CAST("ulink")))
					printf("%s\n", ((char *)
							XML_GET_CONTENT(node->children->next->properties->children)));
			}
		}
