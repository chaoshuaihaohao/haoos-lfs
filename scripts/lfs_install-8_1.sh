...
#8.5.2.2. 添加时区数据
tar -xf $LFS_SRC_DIR/tzdata2021a.tar.gz
...
#8.7. Bzip2-1.0.8
rm bzip2-1.0.8 -rf
tar xf $LFS_SRC_DIR/bzip2-1.0.8.tar.gz
pushd bzip2-1.0.8
patch -Np1 -i $LFS_SRC_DIR/bzip2-1.0.8-install_docs-1.patch
...
#8.12. M4-1.4.18
rm m4-1.4.18 -rf
tar xf $LFS_SRC_DIR/m4-1.4.18.tar.xz
pushd m4-1.4.18
...
#8.15. Tcl-8.6.11
pushd tcl8.6.11
tar -xf $LFS_SRC_DIR/tcl8.6.11-html.tar.gz --strip-components=1
SRCDIR=$(pwd)
cd unix
./configure --prefix=/usr           \
...
#8.31. Gettext-0.21
rm gettext-0.21 -rf
tar xf $LFS_SRC_DIR/gettext-0.21.tar.xz
pushd gettext-0.21
./configure --prefix=/usr    \
            --disable-static \
...
#8.34. Bash-5.1
rm bash-5.1 -rf
tar xf $LFS_SRC_DIR/bash-5.1.tar.gz
pushd bash-5.1
